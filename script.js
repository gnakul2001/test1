...
(function() {
    ...
    var allQuestions = [{
            "question": "1. Full-Form of HTML is _________.",
        ...
        {
            "question": "3. Full-Form of CSS is _________.",
            "choices": [
                "Computing Style Sheet", "Creative Style System", "Cascading Style Sheet", "Creative Styling Sheet"
            ],
            "correctAnswer": 2
        },
    ...
    */
    ...
    function Player(username, password) {
        ...
        this.visits = 0;
        this.finished = true;
        this.answeredAll = false; //added attribute
    ...
    function startQuiz() {
        ...
        actualPlayer.answeredAll = false;
        ...
        showProgress(index);
        showQuestion();
        let totaTime = 210; //210 seconds timer- 3mins
        let min = 0;
        let sec = 0;
        let counter = 0;
        ...
        timer = setInterval(function() {
            ...
            counter++;
            min = Math.floor((totaTime - counter) / 60); //calculating in min
        ...
        }, 1000); //timee set for 1 sec interval
        // time code Ends
    ...
    function makeQuestionButton() {
        // shows button for answered question after clicking 'next'
        var container = document.createElement("div");
        container.classList.add("quizButton");
        container.setAttribute("id", index);
        var question = document.createElement("input");
        var number = index + 1;
        question.setAttribute("value", number);
        question.setAttribute("type", "button");
        question.classList.add("buttons");
        question.addEventListener("click", goToQuestion);
        container.appendChild(question);
        questionButtons.appendChild(container);
    }

    function goToQuestion() {
        // shows question when button is clicked
        formContainer.classList.remove("hide");
        form.innerHTML = "";
        index = this.value - 1;
        var quizItem = allQuestions[index];
    ...
    function checkButtons(index) {
        ...
        var children = document.getElementById("questionButtons").childNodes;
        for (var i = 0; i < children.length; i++) {
            console.log("children[i].getAttribute('id') is " + children[i].getAttribute('id'));
            console.log("index is " + index);
            var number = parseInt(children[i].getAttribute('id'));
            if (number === index) {
                buttonPresent = true;
                console.log("button present with index " + index);
            } else {
                buttonPresent = false;
                console.log(index + " not seen it");
            }
        }
    ...
    function nextQuestion() {
        ...
        var x = questionButtons.childElementCount;
        if (x === 4) {
            actualPlayer.answeredAll = true;
        }
        var buttonPresent = checkButtons(index);
        //only while player has not anwered all questions
        //and button has not already been created
        if (!actualPlayer.answeredAll) {
            if (!buttonPresent) {
                makeQuestionButton();
            }
        }
    ...
    function showScore() {
        ...
        if (actualPlayer.finished) {
            while (questionButtons.hasChildNodes()) {
        ...
        for (var i = 0; i < allQuestions.length; i++) {
            if (storedAnswers[i]) {
                ...
                output = output + '<p>Question ' + (i + 1) + ': <span style="color:red;">Skipped</span></p> ';
            }
        }
